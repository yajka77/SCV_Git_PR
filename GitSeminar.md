...
* В случае, если кнопка `Create Pull Request` не отобразилась, то мы переходим во вкладку `Pull requests`, кликаем по кнопке `New pull request` и выбираем новую ветку в нашем репозитории, после чего кликаем по кнопке *Create pull request*.